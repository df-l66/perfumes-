...
export function Dashboard() {
  const { productos, ventas, clientes, logs, compras } = useAppData();
  ...
  const costoTotal = compras.filter(c => c.estado === 'completada').reduce((sum, c) => sum + c.total, 0);

  // Ganancias (Margen Neto)
  ...
  return (
    <Layout
      ...
      subtitle={`Bienvenido — ${new Date().toLocaleDateString('es-CO', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}`}
    >
      {/* KPI Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
        ...
        <KpiCard
          ...
          subtitle={`Margen neto de utilidad`}
